...
def dedupe_features_ai(features: list[str], known: list[str] | None = None) -> dict:
    ...
    rules = (
        ...
        "separate.\n"
        "BE AGGRESSIVE about consolidating DIFFERENT WORDINGS of the SAME capability. Many "
        "inputs are just paraphrases of a smaller set of capabilities — collapse those "
        "hard so the result is compact (dozens across ~10 products, not one per phrasing). "
        "BUT aggression means merging synonyms, NOT lumping different capabilities into a "
        "generic bucket. NEVER create a grab-bag: e.g. 'Fast-track generation' (speed) / "
        "'Access to web and Discord' (platform access) / 'Faster processing' (speed) / "
        "'Priority support' (support) / 'Enterprise plans' (sales) are ALL DIFFERENT and "
        "must NOT collapse into one 'Support' (or any single) cluster. Merge only when the "
        "underlying capability is truly the same; otherwise keep separate.\n"
        "Two features are the SAME capability — MERGE them — when they differ only by:\n"
        "  • wording, phrasing, or marketing paraphrase of the same capability — "
        "'Generate images' / 'Image generation' / 'AI image creation' / 'Create images "
        "with AI' → 'Image generation'; 'Remove background' / 'Background remover' / 'Auto "
        "background removal' → 'Background removal'; 'Faster processing' / 'Fastest "
        "processing times' / 'Fast-track generation' → 'Faster processing'\n"
        "  • language — 'API access' / 'API 액세스' / 'Developer API'\n"
        ...
        "  • a specific AI MODEL name or version — the capability is just 'access to an AI "
        "model'; the model name/version is a spec, NOT a separate feature. ANY named "
        "generative model → one canonical 'AI 모델 접근'. Use your own knowledge to "
        "recognize model names, INCLUDING ones not listed here: Gemini, GPT-4, Claude, "
        "Sora, Veo, Imagen, Gen-4.5, Runway Gen, Kling, Nano Banana, Flux, Midjourney, "
        "Ideogram, Recraft, Hailuo, MiniMax, Pika, Luma, etc. If a feature is essentially "
        "just a product/model name (often with a version or Pro/Ultra/Max/Mini suffix), "
        "treat it as 'AI 모델 접근'.\n"
        "  • human SUPPORT / onboarding / sales-assist tiers — these are all the same "
        "capability (getting help): 'Priority support' / 'Enterprise-level support' / "
        "'Email support' / 'Dedicated onboarding' / 'Customer success team' / 'Account "
        "manager' → one canonical '지원'.\n"
        ...
        "NOT enough — merge ONLY when it is literally the SAME capability (or the same "
        "model/support family above). Keep SEPARATE genuinely different CAPABILITIES even "
        "if they share a word: 'Basic analytics' vs 'Advanced analytics' can merge to "
        "'Analytics', but 'Image generation' vs 'Image editing' must NOT. These must stay "
        ...
        "Strip only QUANTITY/limit tokens (capped, limited, unlimited, '/day', 'per "
        "month', numbers) from the canonical name, but KEEP meaningful degree/kind "
        "qualifiers (priority, enterprise, standard, email, basic, advanced, pro) so "
        "different levels/kinds stay distinct. Name it in the clearest shortest wording, "
        "matching the dominant language of the inputs.\n"
