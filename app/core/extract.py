...
def dedupe_features_ai(features: list[str], known: list[str] | None = None) -> dict:
    ...
    rules = (
        ...
        "SEPARATE different LEVELS or KINDS of a generic offering — the qualifier is "
        "meaningful, so PRESERVE it in the canonical name: 'Priority support' vs "
        "'Enterprise support' vs 'Email support' vs 'Standard support' are DIFFERENT tiers "
        "(never collapse to bare 'Support'); 'Basic analytics' vs 'Advanced analytics' are "
        "different (never collapse to bare 'Analytics'). Merge only exact same-level wording "
        "or language variants ('Priority support' / '우선 지원'). These must stay "
