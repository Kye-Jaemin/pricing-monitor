...
def extract_bundles_ai(company: str, raw_text: str, anchor: str | None = None) -> dict:
    ...
    prompt = (
        ...
        "discounts, cloud, etc. choice=true if it is a selectable alternative (part of a "
        "'pick N' set), false if always included. list_price = that benefit's standalone/"
        "regular monthly price IF stated (number in same currency), else null. category is "
        "short, e.g. 'Streaming Video', 'Music', 'Mobile/Telecom', 'Cloud Storage', "
        "'Gaming', 'News', 'Shopping', 'Points/Rewards', 'Content', 'Delivery', 'Fitness'. "
